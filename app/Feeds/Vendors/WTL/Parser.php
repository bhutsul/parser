...
class Parser extends HtmlParser
{
    ...
    public const DIMS_REGEXES = [
        ...
        'WDH' => '/(\d+[\.]?\d*)[\',",″]?[\s]?W[\s]?[x,X][\s]?(\d+[\.]?\d*)[\',",″]?[\s]?D[\s]?[x,X]?[\s]?(\d+[\.]?\d*)[\',",″]?[\s]?H/i',
        'LWH' => '/(\d+[\.]?\d*)[\',",″,”]?[\s]?L[\s]?[x,X][\s]?(\d+[\.]?\d*)[\',",″,”]?[\s]?W[\s]?[x,X]?[\s]?(\d+[\.]?\d*)[\',",″,”]?[\s]?H/i',
        'WLH' => '/(\d+[\.]?\d*)[\',",″]?[\s]?W[\s]?[x,X][\s]?(\d+[\.]?\d*)[\',",″]?[\s]?L[\s]?[x,X]?[\s]?(\d+[\.]?\d*)[\',",″]?[\s]?H/i',
        'DIH' => '/(\d+[\.]?\d*)[\',",″]?[\s]?dia[.]?[\s]?[x,X][\s]?(\d+[\.]?\d*)[\',",″]?[\s]H/i',
        'XXX' => '/(\d+[\.]?\d*)[\s]?[x,X][\s]?(\d+[\.]?\d*)[\s]?[x,X]?[\s]?(\d+[\.]?\d*)/i',
    ];
    ...
    private function dimsFromString(string $text): array
    {
        ...
        if (preg_match(self::WEIGHT_REGEX, $search_text, $matches) && isset($matches[1])) {
            $weight = StringHelper::getFloat($matches[1]);
        }

        if (preg_match(self::DIMS_REGEXES['WDH'], $search_text)) {
            $dims = FeedHelper::getDimsRegexp($search_text, [self::DIMS_REGEXES['WDH']], 1, 3, 2);
        } else if (preg_match(self::DIMS_REGEXES['LWH'], $search_text)) {
            $dims = FeedHelper::getDimsRegexp($search_text, [self::DIMS_REGEXES['LWH']], 2, 3, 1);
        } else if (preg_match(self::DIMS_REGEXES['WLH'], $search_text)) {
            $dims = FeedHelper::getDimsRegexp($search_text, [self::DIMS_REGEXES['WLH']], 1, 3, 2);
        } else if (preg_match(self::DIMS_REGEXES['DIH'], $search_text)) {
            $dims = FeedHelper::getDimsRegexp($search_text, [self::DIMS_REGEXES['DIH']]);
        } else if (preg_match(self::DIMS_REGEXES['XXX'], $search_text)) {
            $dims = FeedHelper::getDimsRegexp($search_text, [self::DIMS_REGEXES['XXX']]);
        } else {
            $this->pushAttr($text);
        }
